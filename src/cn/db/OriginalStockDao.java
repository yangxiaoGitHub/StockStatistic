...
public class OriginalStockDao extends OperationDao {
	...
	public void addOriginalData(String sDate, String stockCodes, String changeRates, String turnoverRates) {
		...
		try {
			...
			System.out.println(DateUtils.DateTimeToString(new Date()) + "  " + sDate + "原始股票数据添加失败！");
			log.loger.error(sDate + " 原始股票数据添加失败！");
			e.printStackTrace();
			log.loger.error(e);
		} finally {
			close(state);
	...
	public void updateCodesRatesMD5(Date stockDate, String stockCodesMD5, String changeRatesMD5) {
		...
		try {
			...
			System.out.println(DateUtils.DateTimeToString(new Date()) + "  " + DateUtils.Date2String(stockDate) + "股票数据更新失败！");
			log.loger.error(DateUtils.Date2String(stockDate) + " 股票数据更新失败！");
			e.printStackTrace();
			log.loger.error(e);
		} finally {
			close(state);
