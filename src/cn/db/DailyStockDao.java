...
public class DailyStockDao extends OperationDao {
	...
	public List<Date> analysisStockDatesByStockCode(String stockCode, String startDate, String endDate, String changeFlg) throws SQLException {
		...
		state.setDate(2, new java.sql.Date(DateUtils.String2Date(startDate).getTime()));
		state.setDate(3, new java.sql.Date(DateUtils.String2Date(endDate).getTime()));
	...
	public List<DailyStock> analysisStockByTime(String startDate, String endDate, String changeFlg) throws SQLException {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("select ").append(DailyStock.STOCK_CODE).append(", count(").append(DailyStock.STOCK_CODE).append(") as count_")
		   .append(" from ").append(DailyStock.TABLE_NAME).append(" where ").append(DailyStock.STOCK_DATE)
		   .append(" between ? and ?").append(changeFlgSql).append(" group by ").append(DailyStock.STOCK_CODE)
		   .append(" order by count_ desc, ").append(DailyStock.NUM).append(" asc");
		PreparedStatement state = (PreparedStatement) conn.prepareStatement(sql.toString());
		state.setDate(1, new java.sql.Date(DateUtils.String2Date(startDate).getTime()));
		state.setDate(2, new java.sql.Date(DateUtils.String2Date(endDate).getTime()));
		ResultSet rs = state.executeQuery();
		while (rs.next()) {
	...
	public Map<String, StatisticStock> statisticUpInDailyStock(String stockCode, Date[] startEndDate) throws SQLException {
		...
		sql.append("select ").append(DailyStock.STOCK_CODE).append(", count(").append(DailyStock.STOCK_CODE).append(") as up_number_ from ")
		   ...
		   .append("='1' and ").append(DailyStock.STOCK_DATE).append(" between ? and ?").append(" group by ").append(DailyStock.STOCK_CODE).append(" order by up_number_ desc");
		PreparedStatement state = (PreparedStatement) conn.prepareStatement(sql.toString());
		state.setString(1, stockCode);
		state.setDate(2, new java.sql.Date(startEndDate[0].getTime()));
		state.setDate(3, new java.sql.Date(startEndDate[1].getTime()));
	...
	public Map<String, StatisticStock> statisticDownInDailyStock(String stockCode, Date[] startEndDate) throws SQLException {
		...
		sql.append("select ").append(DailyStock.STOCK_CODE).append(", count(").append(DailyStock.STOCK_CODE).append(") as down_number_ from ")
		   .append(DailyStock.TABLE_NAME).append(" where ").append(DailyStock.STOCK_CODE).append("=? and ").append(DailyStock.CHANGE_FLG)
		   .append("='0' and ").append(DailyStock.STOCK_DATE).append(" between ? and ?").append(" group by ").append(DailyStock.STOCK_CODE).append(" order by down_number_ desc");
		PreparedStatement state = (PreparedStatement) conn.prepareStatement(sql.toString());
		state.setString(1, stockCode);
		state.setDate(2, new java.sql.Date(startEndDate[0].getTime()));
		state.setDate(3, new java.sql.Date(startEndDate[1].getTime()));
	...
	public List<DailyStock> statisticDailyStock() throws SQLException {
		...
		List<DailyStock> dataList = new ArrayList<DailyStock>();
		StringBuffer sql = new StringBuffer();
		sql.append("select ").append(DailyStock.STOCK_CODE).append(", ").append(DailyStock.STOCK_CODE_ENCRYPT).append(", min(")
		   .append(DailyStock.STOCK_DATE).append(") as min_date_ from ").append(DailyStock.TABLE_NAME).append(" group by ")
		   .append(DailyStock.STOCK_CODE).append(" order by min_date_");
		PreparedStatement state = (PreparedStatement) conn.prepareStatement(sql.toString());
		ResultSet rs = state.executeQuery();
		while (rs.next()) {
			DailyStock stock = new DailyStock();
			stock.setStockCode(rs.getString(DailyStock.STOCK_CODE));
			stock.setEncryptStockCode(rs.getString(DailyStock.STOCK_CODE_ENCRYPT));
	...
	public List<DailyStock> statisticDailyData() throws SQLException {
		...
		sql.append("select ").append(DailyStock.STOCK_DATE).append(", count(").append(DailyStock.STOCK_DATE).append(") as count_ from ")
		   .append(DailyStock.TABLE_NAME).append(" group by ").append(DailyStock.STOCK_DATE).append(" order by ").append(DailyStock.STOCK_DATE);
		PreparedStatement state = (PreparedStatement) conn.prepareStatement(sql.toString());
		ResultSet rs = state.executeQuery();
		while (rs.next()) {
			DailyStock data = new DailyStock();
	 ...
	 * 
	 */
	public DailyStock getDailyStockByKey(String sDate, String stockCode) throws SQLException {
		
		DailyStock data = null;
		StringBuffer sql = new StringBuffer();
		sql.append("select ").append(DailyStock.ALL_FIELDS).append(" from ").append(DailyStock.TABLE_NAME)
		   .append(" where ").append(DailyStock.STOCK_DATE).append("=? and ").append(DailyStock.STOCK_CODE).append("=?");
		PreparedStatement state = (PreparedStatement) conn.prepareStatement(sql.toString());
	...
	public List<DailyStock> listDailyData() throws SQLException {
		...
		List<DailyStock> dataList = new ArrayList<DailyStock>();
		StringBuffer sql = new StringBuffer();
		sql.append("select ").append(DailyStock.ALL_FIELDS).append(" from ")
		   .append(DailyStock.TABLE_NAME).append(" order by ").append(DailyStock.NUM);
		PreparedStatement state = (PreparedStatement) conn.prepareStatement(sql.toString());
		ResultSet rs = state.executeQuery();
		while (rs.next()) {
	...
	public boolean isExistInDailyStock(DailyStock data) throws Exception {
		
		int result = 0;
		StringBuffer sql = new StringBuffer();
		sql.append("select count(*) as count_ from ").append(DailyStock.TABLE_NAME).append(" where ")
		   .append(DailyStock.STOCK_CODE).append("=? and ").append(DailyStock.STOCK_DATE).append("=?");
		PreparedStatement state = (PreparedStatement) conn.prepareStatement(sql.toString());
	 ...
	 * 
	 */
	public boolean saveDailyStock(DailyStock data) {
		...
		try {
			...
			System.out.println(DateUtils.DateTimeToString(new Date()) + "  " + DateUtils.Date2String(data.getStockDate()) + "每日股票数据(daily_stock_)添加失败！");
			log.loger.error(DateUtils.Date2String(data.getStockDate()) + " 每日股票数据(daily_stock_)添加失败！");
			e.printStackTrace();
			log.loger.error(e);
		} finally {
			close(state);
	...
	public List<DailyStock> searchStockByTime(String stockCode, String startDate, String endDate, String changeFlg) throws SQLException {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("select ").append(DailyStock.ALL_FIELDS).append(" from ").append(DailyStock.TABLE_NAME)
							 .append(" where ").append(DailyStock.STOCK_CODE).append("=? ").append(dateSql)
							 .append(changeFlgSql).append(" order by ").append(DailyStock.STOCK_DATE).append(" desc");
		PreparedStatement state = (PreparedStatement) conn.prepareStatement(sql.toString());
		state.setString(1, stockCode);
		if (!dateSql.equals("")) {
			state.setDate(2, new java.sql.Date(DateUtils.String2Date(startDate).getTime()));
	...
	public Date[] getMinMaxDate() throws SQLException {
		...
		String sql = "select min(" + DailyStock.STOCK_DATE + ") as min_date_, max(" + DailyStock.STOCK_DATE + ") as max_date_ from " + DailyStock.TABLE_NAME;
		PreparedStatement state = (PreparedStatement) conn.prepareStatement(sql);
		ResultSet rs = state.executeQuery();
		while (rs.next()) {
